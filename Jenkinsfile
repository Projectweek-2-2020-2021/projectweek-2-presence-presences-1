    ...
    stages {
        stage('Build Docker Image for TEST and deploy to TEST'){
            steps{
               ...
               sh 'docker service create -p 15007:8080 --name ${PROJECT_NAME}_DEV  ${PROJECT_NAME}:dev || docker service update --image ${PROJECT_NAME}:dev ${PROJECT_NAME}_DEV'
        ...
        stage('Deployment to PROD') {
            steps {
                sh 'docker service create -p 14007:8080 --name ${PROJECT_NAME}_PROD  ${PROJECT_NAME}:prod || docker service update --image ${PROJECT_NAME}:prod ${PROJECT_NAME}_PROD'
